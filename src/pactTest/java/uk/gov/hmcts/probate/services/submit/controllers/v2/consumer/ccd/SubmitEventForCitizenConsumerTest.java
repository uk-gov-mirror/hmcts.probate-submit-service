...
public class SubmitEventForCitizenConsumerTest extends AbstractProbateSubmitServicePact {
    ...
    RequestResponsePact submitEventForCitizen(PactDslWithProvider builder) throws Exception {
        // @formatter:off
        return builder
            .given("A Submit Event for a Citizen is requested", setUpStateMapForProviderWithCaseData(APPLY_FOR_GRANT))
            .uponReceiving("A Submit Event for a Citizen is requested")
            .path("/citizens/"
                + caseworkerUsername
                + "/jurisdictions/"
                + jurisdictionId
